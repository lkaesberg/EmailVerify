  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("email")
        .setDescription(
          "register blacklisted emails (add multiple separated by ','), (pass \"-\" to clear)"
        )
    ),
      ...
      async (serverSettings) => {
        if (!blacklist) {
          await interaction.reply(
            "Blacklisted names: " + serverSettings.blacklist.join(", ")
          );
        } else if (blacklist === "-") {
          ...
          const blacklistedNames = serverSettings.blacklist.concat(
            blacklist.split(",").map((name) => name.trim())
          );
