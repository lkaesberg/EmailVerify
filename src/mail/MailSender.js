...
module.exports = class MailSender {
  ...
  async sendEmail(
    ...
    const isGuildInteraction =
      typeof message.guildId !== "undefined" && message.guildId !== null;
    const userId = message.user ? message.user.id : message.author.id;
    const serverId = isGuildInteraction
      ? message.guildId
      : this.userGuilds.get(userId).id;
          ...
          if (!options.suppressReply) {
            ...
            if (isGuildInteraction) {
              if (message.deferred || message.replied) {
                ...
                await message
                  .reply({ content: negative, flags: MessageFlags.Ephemeral })
                  .catch(() => {});
              }
            } else {
          ...
          if (!options.suppressReply) {
            ...
            if (isGuildInteraction) {
              if (message.deferred || message.replied) {
                ...
                await message
                  .reply({ content: positive, flags: MessageFlags.Ephemeral })
                  .catch(() => {});
              }
            } else {
